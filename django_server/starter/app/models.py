...
def rate_book(book_id, user_id, rating):
    try:
        ...
        ok = False
        raise Exception('Book with id {book_id} doesn\'t exist')
    try:
        user = UserModel.objects.get(id=user_id)
    except:
        raise Exception('User with id {user_id} doesn\'t exist')
...
def delete_rating(book_id, user_id):
    try:
        book = Book.objects.get(id=book_id)
    except:
        ok = False
    ...
    HasRead.objects.filter(user=user, book=book).delete()